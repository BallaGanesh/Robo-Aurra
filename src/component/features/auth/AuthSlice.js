...
export const registerUser = createAsyncThunk(
  ...
  async (formData, thunkAPI) => {
    try {
      ...
      const response = await axios.post("http://192.168.0.66:5000/api/users/register", formData, {
...
export const loginUser = createAsyncThunk(
  ...
  async (formData, thunkAPI) => {
    try {
      // Example endpoint — replace with your backend API
      const response = await axios.post("http://192.168.0.66:5000/api/users/login", formData);
      console.log(response);
      return response.data;
    } catch (error) {
      return thunkAPI.rejectWithValue(
